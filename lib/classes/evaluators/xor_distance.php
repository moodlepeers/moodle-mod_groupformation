...
class mod_groupformation_xor_distance implements mod_groupformation_idistance {

    /**
     * Normalizes distance for each dimension (INTERNAL method)
     * with XOR operator (all values larger 0 are assumed to be 1)
     *
    ...
    private function get_distance(mod_groupformation_criterion $cr1, mod_groupformation_criterion $cr2) {
        ...
        for ($i = 0; $i < count($cr1->get_values()); $i++) {
            $v1 = $cr1->get_value($i) > 0.0;
            $v2 = $cr2->get_value($i) > 0.0;
            $distance += $v1 ^ $v2;
        }
        return $distance;
    }
    ...
    public function normalized_distance(mod_groupformation_criterion $c1, mod_groupformation_criterion $c2) {
        ...
        $result = (float($this->get_distance($c1, $c2)) / count($c1->get_values()));
        return $result;
    }
